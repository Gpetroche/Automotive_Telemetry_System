...
def Iniciar():
    ...
    isRun = True   
    thread.start() 
    anim = animacion.FuncAnimation(fig, plotData,  fargs=(muestraD,lines),interval = 1200, blit = False )
    plt.show()
...
def DatosA():
    ...
    while (isRun):
        ...
        RawString = arduino.readline()                                  #Datos leidos sin tratar enviados por Arduino.
        RawDataIn = RawString.decode("utf-8").strip()
        RawDataIn = RawDataIn.split(",")
        

        if len(RawDataIn) == 4:
            del RawDataIn[0:2]  #Se eliminan los dos primero elementos del vector (continenen simplemente comas)
            del RawDataIn[-1]   #Se elimina el último elemento del vector (Carry return)
        elif len(RawDataIn) == 3:
            del RawDataIn[0]
            del RawDataIn[-1]   #Se elimina el último elemento del vector (Carry return)
